...
pub fn create_update_entitiy(model: &InternalModel) -> String {
    let fields: String = match &model.properties {
        None => "".to_string(),
        Some(properties) => {
            properties.keys().map(|p| format!("{} = ?", p)).collect::<Vec<_>>().join(", ")
        }
    };
...
mod tests {
    ...
    fn test_delete_entity() {
        ...
        assert_eq!(expected, create_delete_entitiy(&InternalModel::new_with_props("Order".to_string(), props)));
    }
    ...
    fn test_update_entity_with_multiple_properties() {
        ...
        assert_eq!(expected, create_update_entitiy(&InternalModel::new_with_props("Order".to_string(), props)));
    }
